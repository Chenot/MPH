...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='E:\\OneDrive - ISAE-SUPAERO\\MPH\\tasks\\MentalRotation_MRT\\MentalRotation_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    exampText = visual.TextStim(win=win, name='exampText',
        text=None,
        font='Arial',
        pos=[0, -0.25], height=0.04, wrapWidth=1.5, ori=0, 
        color='white', colorSpace='rgb', opacity=1, 
        languageStyle='LTR',
        depth=0.0);
    ...
    cross = visual.TextStim(win=win, name='cross',
        ...
        languageStyle='LTR',
        depth=-1.0);
    textPuzzle_2 = visual.TextStim(win=win, name='textPuzzle_2',
        text=None,
        font='Arial',
        ...
        languageStyle='LTR',
        depth=-2.0);
    textKeys_2 = visual.TextStim(win=win, name='textKeys_2',
        text=None,
        font='Arial',
    ...
    # keep track of which components have finished
    Instruction1Components = [helloText, helloKey, nametask]
    ...
    while continueRoutine:
        ...
        if helloKey.status == STARTED and not waitOnFlip:
            theseKeys = helloKey.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
    ...
    # keep track of which components have finished
    Instruction2Components = [exampText, exampKey, exampIm, nametask_2]
    ...
    while continueRoutine:
        ...
        if exampKey.status == STARTED and not waitOnFlip:
            theseKeys = exampKey.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
    ...
    for thisLoop in loop:
        ...
        # Run 'Begin Routine' code from code_textPuzzle_2
        # Set the text for the first set of instructions
        textPuzzle_2.setText(instructions['Text_puzzle'] + str(trial) + "/100")
        textKeys_2.setText('')
        # Run 'Begin Routine' code from code_textKeys_2
        textKeys_2.setText(instructions['Text_keys'])
        
        # keep track of which components have finished
        fixation_crossComponents = [blank, cross, textPuzzle_2, textKeys_2]
        ...
        while continueRoutine:
            ...
            # *textPuzzle_2* updates
            
            # if textPuzzle_2 is starting this frame...
            if textPuzzle_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                textPuzzle_2.frameNStart = frameN  # exact frame index
                textPuzzle_2.tStart = t  # local t and not account for scr refresh
                textPuzzle_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(textPuzzle_2, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'textPuzzle_2.started')
                # update status
                textPuzzle_2.status = STARTED
                textPuzzle_2.setAutoDraw(True)
            
            # if textPuzzle_2 is active this frame...
            if textPuzzle_2.status == STARTED:
                # update params
                pass
            
            # *textKeys_2* updates
            
            # if textKeys_2 is starting this frame...
            if textKeys_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                textKeys_2.frameNStart = frameN  # exact frame index
                textKeys_2.tStart = t  # local t and not account for scr refresh
                textKeys_2.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(textKeys_2, 'tStartRefresh')  # time at next scr refresh
                # update status
                textKeys_2.status = STARTED
                textKeys_2.setAutoDraw(True)
            
            # if textKeys_2 is active this frame...
            if textKeys_2.status == STARTED:
                # update params
                pass
            if thisExp.status == FINISHED or endExpNow:
                endExperiment(thisExp, win=win)
        ...
        stimulus_type = f"stimulus_{imagefile}"
        ...
        while continueRoutine:
            ...
            if trialResp.status == STARTED and not waitOnFlip:
                theseKeys = trialResp.getKeys(keyList=['d', 'k'], ignoreKeys=None, waitRelease=False)
        ...
        answer_corr = f"response_{trialResp.corr}"
