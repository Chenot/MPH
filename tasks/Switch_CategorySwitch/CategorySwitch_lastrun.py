...
expName = 'CategorySwitch'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='E:\\OneDrive - ISAE-SUPAERO\\MPH_test\\tasks\\Switch_CategorySwitch\\CategorySwitch_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    scenario_df = pd.read_csv('category_switch_task_scenario.csv')
    ...
    if language == "English":
        instructions = {
            'name_task' : 'CATEGORY SWITCH',
            'Introduction': "Welcome to the 'Category Switch' task.\n\nIn this task, you will see one of several words, appearing one at a time.\n\nSometimes you will be asked to respond to whether the word represents a LIVING BEING or not; at other times, you will be asked to respond to whether the object described by the word is LARGER or SMALLER than a basketball.",
            'Practice_Living_Only': "If you see a heart on the screen, you will be asked to respond to whether the word represents a LIVING BEING or not.\n\nIf the word represents a LIVING BEING, press the 'k' key.\nIf the word does NOT represent a living being, press the 'd' key.\n\nPress the space bar to begin the practice session for living beings only.",
            'Practice_Size_Only': "If you see an arrowcross on the screen, you will be asked to respond to the SIZE of the object described by the word.\n\nIf the object is LARGER than a basketball, press the 'k' key.\nIf the object is SMALLER than a basketball, press the 'd' key.\n\nPress the space bar to begin the practice session for size only.",
            'Practice_Both': "You will now practice responding to both LIVING/NON-LIVING and SIZE categories.\n\nPay attention to the cues and respond to the words as quickly as possible without making too many errors.\n\nPress the space bar to begin the practice session.",
            'Start_Actual_Task': "The training is over. Let's now do the reak task.\n\nThis consists of 4 rounds. Each round lasts about 2 minutes.\n\nPress the space bar to start.",
        ...
        instructions = {
            'name_task' : 'CATEGORY SWITCH',
            'Introduction': "Bienvenu dans la tâche de 'Category Switch'.\n\nDans cette tâche, vous verrez un mot parmi plusieurs, apparaissant un à la fois.\n\nParfois, on vous demandera de répondre à la question de savoir si le mot représente un ÊTRE VIVANT ou non ; à d'autres moments, on vous demandera de répondre à la question de savoir si l'objet décrit par le mot est PLUS GRAND ou PLUS PETIT qu'un ballon de basket.",
            'Practice_Living_Only': "Si vous voyez un cœur à l'écran, il vous sera demandé de répondre à la question de savoir si le mot représente un ÊTRE VIVANT ou non.\n\nSi le mot représente un ÊTRE VIVANT, appuyez sur la touche 'k'.\nSi le mot NE représente PAS un être vivant, appuyez sur la touche 'd'.\n\nAppuyez sur la barre d'espace pour commencer la session d'entraînement pour les êtres vivants uniquement.",
            'Practice_Size_Only': "Si vous voyez un croix flèchée à l'écran, il vous sera demandé de répondre à la question de la TAILLE de l'objet décrit par le mot.\n\nSi l'objet est PLUS GRAND qu'un ballon de basket, appuyez sur la touche 'k'.\nSi l'objet est PLUS PETIT qu'un ballon de basket, appuyez sur la touche 'd'.\n\nAppuyez sur la barre d'espace pour commencer la session d'entraînement pour la taille uniquement.",
            'Practice_Both': "Vous allez maintenant vous entraîner à répondre à la fois aux catégories VIVANT/NON-VIVANT et TAILLE.\n\nFaites attention aux indices et répondez aux mots aussi vite que possible sans faire trop d'erreurs.\n\nAppuyez sur la barre d'espace pour commencer la session d'entraînement.",
            'Start_Actual_Task': "L'entraînement est terminé. Passons à la vraie tâche\n\nCette dernière se déroulera sur 4 tours. Chaque tour durera environ 2 minutes.\n\nAppuyez sur la barre d'espace pour commencer.",
    ...
    stimulus_image_practice = visual.ImageStim(
        ...
        ori=0.0, pos=(0, 0.15), size=(0.12, 0.12),
    ...
    send_marker('CategorySwitch_start')
    ...
    practiceblocks = data.TrialHandler(nReps=1.0, method='sequential', 
        ...
        trialList=data.importConditions('category_switch_task_scenario.csv', selection=scenario_df[(scenario_df['block'] == 'practice') & (scenario_df['block_n'] == block_index)].index.tolist()),
    ...
    for thisPracticeblock in practiceblocks:
        ...
        if current_block_type == 'living':
            text_practice.text = instructions['Practice_Living_Only']
        elif current_block_type == 'size':
            text_practice.text = instructions['Practice_Size_Only']
        ...
        for thisPracticeTrial in practiceTrials:
            ...
            if language == "English":
                if current_block_type == 'living':
                    instruction_text_component_practice.text = word_eng
                ...
                if current_block_type == 'living':
                    instruction_text_component_practice.text = word_fr
                    instruction_d.text = 'd\nnon vivant'
                    instruction_k.text = 'k\nvivant'
                elif current_block_type == 'size':
                    instruction_text_component_practice.text = word_fr
                    instruction_d.text = 'd\nplus petit que balle de basket'
                    instruction_k.text = 'k\nplus grand que balle de basket'
                elif current_block_type == 'both':
                    instruction_text_component_practice.text = word_fr
                    instruction_d.text = 'd\nnon vivant\nplus petit que balle de basket'
                    instruction_k.text = 'k\nvivant\nplus grand que balle de basket'
            ...
            while continueRoutine:
                ...
                if instruction_text_component_practice.status == NOT_STARTED and frameN >= iti_60hz+soa_60hz:
                    # keep track of start time/frame for later
                ...
                if stimulus_image_practice.status == NOT_STARTED and frameN >= iti_60hz:
                    # keep track of start time/frame for later
                ...
                if frameN == iti_60hz+soa_60hz:
                    ...
                    send_marker(instruction + "_" + trial_type + "_" + word_eng)
                    print(instruction + "_" + trial_type + "_" + word_eng)
                if thisExp.status == FINISHED or endExpNow:
                    endExperiment(thisExp, win=win)
                    return
            ...
            # Run 'End Routine' code from LSL_response_practice
            if not response_practice.corr:  # Check if the response is incorrect
                send_marker("0_" + instruction + "_" + trial_type + "_" + word_eng)
            else:
                send_marker("1_" + instruction + "_" + trial_type + "_" + word_eng)
            ...
            if not response_practice.corr:  # Check if the response is incorrect
                ...
                #send_marker("0_" + instruction + "_" + trial_type + "_" + word_eng)
                #print("0_" + instruction + "_" + trial_type + "_" + word_eng)
            else:
                feedback_message = ''  # No feedback if correct
                #send_marker("1_" + instruction + "_" + trial_type + "_" + word_eng)
                #print("1_" + instruction + "_" + trial_type + "_" + word_eng)
    ...
    testblocks = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('category_switch_task_scenario.csv', selection=scenario_df[(scenario_df['block'] == 'test') & (scenario_df['block_n'] == block_index)].index.tolist()),
    ...
    for thisTestblock in testblocks:
        ...
        for thisTestTrial in testTrials:
            ...
            # Run 'Begin Routine' code from code_instruction_text_3
            # Check if expInfo exists and create the language variable
            if 'expInfo' not in globals():
                expInfo = {}
            ...
            if language == "English":
                instruction_text_component.text = word_eng
            else:
                instruction_text_component.text = word_fr
            ...
            while continueRoutine:
                ...
                if instruction_text_component.status == NOT_STARTED and frameN >= iti_60hz+soa_60hz:
                    # keep track of start time/frame for later
                ...
                if stimulus_image.status == NOT_STARTED and frameN >= iti_60hz:
                    # keep track of start time/frame for later
            ...
            if not response_practice.corr:  # Check if the response is incorrect
                feedback_message = ''  # Set the feedback message to a red cross
            else:
                feedback_message = ''  # No feedback if correct
    ...
    # Run 'Begin Routine' code from LSL_CategorySwitch_end
    send_marker('CategorySwitch_end')
    # Run 'Begin Routine' code from code_InstructionThanks
