...
expName = 'Similarities'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Google Drive\\Professionnel\\3_Post-doc_ISAE-MPH\\Experience_MPH\\project\\code\\PsychopyTasks\\Verbal_Similarities\\Similarities_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    if language == "English":
        instructions = {
            'name_task' : 'SIMILARITIES',
            'Text_instructions': (
                "Welcome to the Similarity Task.\n\n"
                "In this task, you will see two words on the screen.\n"
                "Your objective is to find and write a WORD or a DESCRIPTION that best describes the similarity between these two words. "
                "If you are unsure, you can also provide a second description. "
                "You will have between 30 and 60 seconds to answer depending on the items.\n\n"
                "For example: HOT - COLD\n\n"
                "The word that would best describe the similarity is 'temperature'.\n"
                "An alternative would be 'can be measured by a thermometer'.\n"
                "Note that 'opposite' or 'different' would not be considered as a valid answer as this does not describe the similarity.\n\n"
            ...
            'Placeholder1': "Type first word/description here...",
            'Placeholder2': "Type second word/description here (optional)..."
        }
    else:  # Default to French if any issues
        instructions = {
            'name_task' : 'SIMILARITÉS',
            'Text_instructions': (
                "Bienvenue dans la tâche de Similarité.\n\n"
                "Dans cette tâche, vous verrez deux mots à l'écran.\n"
                "Votre objectif est de trouver et écrire un MOT ou une DESCRIPTION qui décrit le mieux la similarité entre ces deux mots. "
                "Si vous n'êtes pas sûr, vous pouvez également fournir une deuxième description. "
                "Vous aurez entre 30 et 60 secondes pour répondre en fonction des items.\n\n"
                "Par exemple: CHAUD - FROID\n\n"
                "Le mot qui décrirait le mieux la similarité est 'température'.\n"
                "Une alternative serait 'peut être mesuré par un thermomètre'.\n"
                "Notez que 'inverse' ou 'différents' ne sont pas des réponses valides car ils ne décrivent pas ce qui est similaire.\n\n"
            ...
            'Placeholder1': "Tapez le premier mot/la première description ici...",
            'Placeholder2': "Tapez le deuxième mot/la deuxième description ici (facultatif)..."
        }
    ...
    while continueRoutine:
        ...
        # Run 'Each Frame' code from Activate_Mouse
        # Activate mouse
    ...
    send_marker("start_Similarities")
    thisExp.nextEntry()
    ...
    trials = data.TrialHandler(nReps=1, method='sequential', 
        ...
        trialList=data.importConditions('similarities.csv'),
    ...
    for thisTrial in trials:
        ...
        TextTrial.setText(instructions['Text_trial'] + str(trial) + "/25")
        ...
        textbox_2.setText('')
        textbox_2.setPlaceholder(instructions['Placeholder2'])
        # Run 'Begin Routine' code from LSL_start_trial
        marker = f"stimulus_{trial}"
        send_marker(marker)  # Function to send the LSL marker
        # keep track of which components have finished
        TrialComponents = [stimulus, textbox_1, textbox_2, button_next, text_timer, TextTrial]
    ...
    # Run 'Begin Routine' code from LSL_Similarities_end
    send_marker('Similarities_end')
    end_task.keys = []
    end_task.rt = []
    _end_task_allKeys = []
    # keep track of which components have finished
