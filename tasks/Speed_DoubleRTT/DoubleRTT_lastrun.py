...
expName = 'DoubleRTT'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Google Drive\\Professionnel\\3_Post-doc_ISAE-MPH\\Experience_MPH\\project\\code\\PsychopyTasks\\Speed_DoubleRTT\\DoubleRTT_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    scenario_df = pd.read_csv('double_rtt_scenario.csv')
    ...
    if language == "English":
        instructions = {
            'name_task' : 'DOUBLE REACTION TIME',
            'Text_instructions': "Welcome to the Double Reaction Time task.\n\nIn this task your goal is to press the left or arrow key depending of the direction of the arrow presented to you.\n\nThe goal is to be as fast as possible.\nFor this, you will have to use your dominant hand (for example, right hand if you are right-handed).\n\nFirst, we shall have a practice session.\n\nPress the space bar to begin the practice session.",
            'Text_start_task': "Now we shall begin the actual task.\n\nReady?\n\nPress the 'space bar' to begin.",
    ...
    # Run 'Begin Routine' code from LSL_DoubleRTT_start
    send_marker('DoubleRTT_start')
    ...
    practiceTrials = data.TrialHandler(nReps=1, method='sequential', 
        ...
        trialList=data.importConditions('double_rtt_scenario.csv', selection=selected_rows_practice),
    ...
    for thisPracticeTrial in practiceTrials:
        ...
        response.rt = []
        _response_allKeys = []
        arrow.setImage(stimulus)
        # keep track of which components have finished
        mainComponents = [bgMain, response, arrow]
        ...
        while continueRoutine:
            ...
            # if bgMain is starting this frame...
            if bgMain.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
                bgMain.frameNStart = frameN  # exact frame index
                bgMain.tStart = t  # local t and not account for scr refresh
                bgMain.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(bgMain, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'bgMain.started')
                # update status
                bgMain.status = STARTED
                bgMain.setAutoDraw(True)
            
            # if bgMain is active this frame...
            if bgMain.status == STARTED:
                # update params
                pass
            ...
            if response.status == NOT_STARTED and frameN >= isi_frame60hz:
                # keep track of start time/frame for later
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['left','right'], ignoreKeys=None, waitRelease=False)
            ...
            if arrow.status == NOT_STARTED and frameN >= isi_frame60hz:
                # keep track of start time/frame for later
    ...
    # Run 'End Routine' code from LSL_start_block
    send_marker("start_task")
    thisExp.nextEntry()
    ...
    testblocks = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('double_rtt_scenario.csv'),
        seed=scenario_df[(scenario_df['block'] == 'test') & (scenario_df['block_n'] == block_index)].index.tolist(), name='testblocks')
    ...
    for thisTestblock in testblocks:
        ...
        testTrials = data.TrialHandler(nReps=1, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions('double_rtt_scenario.csv', selection=scenario_df[(scenario_df['block'] == 'test') & (scenario_df['block_n'] == block_index)].index.tolist()),
        ...
        # Run 'End Routine' code from check_block_counter
    ...
    # Run 'Begin Routine' code from LSL_DoubleRTT_end
    send_marker('DoubleRTT_end')
    # Run 'Begin Routine' code from code_InstructionText4
