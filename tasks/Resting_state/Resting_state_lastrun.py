...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\OneDrive - ISAE-SUPAERO\\MPH\\tasks\\Resting_state\\Resting_state_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    # create speaker 'sound_EC'
    deviceManager.addDevice(
        deviceName='sound_EC',
        deviceClass='psychopy.hardware.speaker.SpeakerDevice',
        index=-1
    )
    # create speaker 'sound_EO'
    deviceManager.addDevice(
        deviceName='sound_EO',
        deviceClass='psychopy.hardware.speaker.SpeakerDevice',
        index=-1
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    if language == "English":
        instructions = {
            ...
            'Text_instructions': (
                ...
                "In this, you will be at rest (not performing any task), and will alternate between eyes open and eyes closed\n\n"
        ...
        instructions = {
            ...
            'Text_instructions': (
                ...
                "Dans celle-ci, vous serez au repos (vous n'effectuerez pas de tâche à proprement parler) et vous alternerez entre les yeux ouverts et les yeux fermés.\n\n"
                "Essayez de vous détendre, de ne pas bouger et de laisser libre cours à vos pensées tout en restant éveillé.\n\n"
    ...
    text_EC = visual.TextStim(win=win, name='text_EC',
        ...
        languageStyle='LTR',
        depth=0.0);
    sound_EC = sound.Sound(
        'A', 
        secs=-1, 
        stereo=True, 
        hamming=True, 
        speaker='sound_EC',    name='sound_EC'
    )
    sound_EC.setVolume(1.0)
    ...
    cross = visual.TextStim(win=win, name='cross',
        ...
        languageStyle='LTR',
        depth=0.0);
    sound_EO = sound.Sound(
        'A', 
        secs=-1, 
        stereo=True, 
        hamming=True, 
        speaker='sound_EO',    name='sound_EO'
    )
    sound_EO.setVolume(1.0)
    ...
    # create starting attributes for startInst
    startInst.keys = []
    startInst.rt = []
    _startInst_allKeys = []
    ...
    # Run 'Begin Routine' code from LSL_RS_start
    send_marker('Resting-State_start')
    # Run 'Begin Routine' code from code_nametask
    nametask.setText(instructions['name_task'])
    
    # keep track of which components have finished
    instructions1Components = [InstructionsText, startInst, nametask]
    ...
    loop_RS = data.TrialHandler(nReps=5.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=[None],
        seed=None, name='loop_RS')
    thisExp.addLoop(loop_RS)  # add the loop to the experiment
    thisLoop_RS = loop_RS.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisLoop_RS.rgb)
    if thisLoop_RS != None:
        for paramName in thisLoop_RS:
            globals()[paramName] = thisLoop_RS[paramName]
    
    for thisLoop_RS in loop_RS:
        currentLoop = loop_RS
        ...
        sound_EC.setSound('ressources/EC_fr.wav', secs=3, hamming=True)
        sound_EC.setVolume(1.0, log=False)
        sound_EC.seek(0)
        # Run 'Begin Routine' code from LSL_EC
        send_marker("audio_EC")
        
        # Initialize the clock at the beginning of the routine
        routine_clock = core.Clock()
        routine_clock.reset()
        
        # Initialize the flag to track if the marker has been sent
        marker_sent = False
        
        # keep track of which components have finished
        Eyes_closedComponents = [text_EC, sound_EC]
        ...
        while continueRoutine and routineTimer.getTime() < 32.0:
            ...
            if text_EC.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if text_EC.status == STARTED:
                ...
                if tThisFlipGlobal > text_EC.tStartRefresh + 32-frameTolerance:
            ...
            # if sound_EC is starting this frame...
            if sound_EC.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                sound_EC.frameNStart = frameN  # exact frame index
                sound_EC.tStart = t  # local t and not account for scr refresh
                sound_EC.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('sound_EC.started', tThisFlipGlobal)
                # update status
                sound_EC.status = STARTED
                sound_EC.play(when=win)  # sync with win flip
            
            # if sound_EC is stopping this frame...
            if sound_EC.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_EC.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
                    sound_EC.tStop = t  # not accounting for scr refresh
                    sound_EC.tStopRefresh = tThisFlipGlobal  # on global time
                    sound_EC.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'sound_EC.stopped')
                    # update status
                    sound_EC.status = FINISHED
                    sound_EC.stop()
            # update sound_EC status according to whether it's playing
            if sound_EC.isPlaying:
                sound_EC.status = STARTED
            elif sound_EC.isFinished:
                sound_EC.status = FINISHED
            # Run 'Each Frame' code from LSL_EC
            # Get the current time elapsed
            elapsed_time = routine_clock.getTime()
            
            # Check if the elapsed time matches or exceeds 2 seconds and the marker has not been sent yet
            if elapsed_time >= 2.0 and not marker_sent:
                # Send the marker with the stimulus type
                send_marker("EC_begin")
                # Set the flag to True to indicate the marker has been sent
                marker_sent = True
        ...
        sound_EC.pause()  # ensure sound has stopped at end of Routine
        # Run 'End Routine' code from LSL_EC
        send_marker("EC_end")
        ...
        sound_EO.setSound('ressources/EO_fr.wav', secs=3, hamming=True)
        sound_EO.setVolume(1.0, log=False)
        sound_EO.seek(0)
        # Run 'Begin Routine' code from LSL_EO
        send_marker("audio_EO")
        ...
        # keep track of which components have finished
        Eyes_openComponents = [cross, sound_EO]
        ...
        while continueRoutine and routineTimer.getTime() < 32.0:
            ...
            if cross.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > cross.tStartRefresh + 32-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # if sound_EO is starting this frame...
            if sound_EO.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                sound_EO.frameNStart = frameN  # exact frame index
                sound_EO.tStart = t  # local t and not account for scr refresh
                sound_EO.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('sound_EO.started', tThisFlipGlobal)
                # update status
                sound_EO.status = STARTED
                sound_EO.play(when=win)  # sync with win flip
            
            # if sound_EO is stopping this frame...
            if sound_EO.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sound_EO.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
                    sound_EO.tStop = t  # not accounting for scr refresh
                    sound_EO.tStopRefresh = tThisFlipGlobal  # on global time
                    sound_EO.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'sound_EO.stopped')
                    # update status
                    sound_EO.status = FINISHED
                    sound_EO.stop()
            # update sound_EO status according to whether it's playing
            if sound_EO.isPlaying:
                sound_EO.status = STARTED
            elif sound_EO.isFinished:
                sound_EO.status = FINISHED
            # Run 'Each Frame' code from LSL_EO
            # Get the current time elapsed
            elapsed_time = routine_clock.getTime()
        ...
        sound_EO.pause()  # ensure sound has stopped at end of Routine
        # Run 'End Routine' code from LSL_EO
        send_marker("EO_end")
    ...
    # completed 5.0 repeats of 'loop_RS'
    ...
    # Run 'Begin Routine' code from LSL_RS_end
    send_marker('RS_end')
