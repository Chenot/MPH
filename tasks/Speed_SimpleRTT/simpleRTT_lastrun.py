...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Google Drive\\Professionnel\\3_Post-doc_ISAE-MPH\\Experience_MPH\\project\\code\\PsychopyTasks\\Speed_SimpleRTT\\simpleRTT_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    scenario_df = pd.read_csv('keyboard_reaction_time_scenario.csv')
    ...
    if language == "French":
        instructions = {
            'name_task' : 'TEMPS DE REACTION (clavier)',
            'Text_instructions': "Bienvenue dans la tâche de temps de réaction au clavier.\n\nDans cette tâche, votre objectif est d'appuyer sur la barre d'espace lorsque la croix apparaît.\n\nL'objectif est de répondre aussi rapidement que possible.\nPour cela, vous devrez utiliser votre main dominante (par exemple, la main droite si vous êtes droitier).\n\nD'abord, nous allons faire une session d'entraînement.\nAppuyez sur la barre d'espace pour commencer.",
    ...
    # Run 'Begin Experiment' code from code_debug
    print(language)
    ...
    # Run 'Begin Routine' code from LSL_SimpleRTT_start
    send_marker('SimpleRTT_start')
    ...
    practiceTrials = data.TrialHandler(nReps=1, method='sequential', 
        ...
        trialList=data.importConditions('keyboard_reaction_time_scenario.csv', selection=selected_rows_practice),
    ...
    for thisPracticeTrial in practiceTrials:
        ...
        _response_allKeys = []
        # keep track of which components have finished
        mainComponents = [bgMain, targ, response]
        ...
        while continueRoutine:
            ...
            # if targ is starting this frame...
            if targ.status == NOT_STARTED and frameN >= isi_frame60hz:
                # keep track of start time/frame for later
            ...
            if response.status == STARTED and not waitOnFlip:
                theseKeys = response.getKeys(keyList=['space'], ignoreKeys=None, waitRelease=False)
    ...
    testblocks = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('keyboard_reaction_time_scenario.csv', selection=scenario_df[(scenario_df['block'] == 'test') & (scenario_df['block_n'] == block_index)].index.tolist()),
    ...
    # Run 'Begin Routine' code from LSL_SimpleRTT_end
    send_marker('SimpleRTT_end')
