...
# Bonuses
num_bonus_pairs = int(game_duration/8)  # Approximately one pair of bonus every 8 second.
...
# Mines
num_mines = int(game_duration/12) # Approximately one mine every 12 second.
...
def generate_scenario_yaml():
    ...
    for i, mine_type in enumerate(shuffled_mine_types[:-1]):
        ...
        # Add expected_key for type-2 mines
        if mine_type == "type-2":
            events.append({
                "timestamp": time_str,
                "events": [{
    ...
    if last_mine_type == "type-2":
        events.append({
            "timestamp": format_time(last_mine_appearance),
            "events": [{
                "event": "pop_mine",
                "id": f"m{mine_id}",
                "location": shuffled_mine_positions[-1],
    ...
    # First, generate all bonuses except the last one
    ...
    for i, bonus_type in enumerate(shuffled_bonus_types[:-1]):  # Exclude the last bonus
        ...
        # Second symbol of the pair (pop_bonus_valid for valid bonuses with "$")
    ...
    # Second symbol of the last pair (pop_bonus_valid for valid bonuses with "$")
